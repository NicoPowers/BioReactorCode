...
float frequency, period, acceleration, steps, mm = 0.0, stepsToGoBack, newMaxSpeed, distanceFromHome = 0.0, repeatingDistance = 0.0, offset = 0.0;
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    if (decision == 'r') {
      ...
      offset = input.substring(1).toFloat();
...
void travelOutwards ( float mm ) {
  ...
  stepper.setAcceleration(acceleration - offset);
