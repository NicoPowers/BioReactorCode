...
float pressure_psi;
float pressure_mmHg;
float initial_pressure_pa;
float final_pressure_pa;
float pressure_difference;
float voltage;

float tubing_radius = 1.55; // mm
int tubing_length = 570; // mm
float water_viscosity = 0.000089; // pa*s
float theoretical_flow_rate;
float experimental_flow_rate;

int pressure_transducer_1 = A1;
int pressure_transducer_2 = A0;
int flow_rate_sensor = A2;

int final_pressure_analog;
int intial_pressure_analog;
int experimental_flow_rate_analog;
...
void loop() {
  ...
  intial_pressure_analog = analogRead(pressure_transducer_1);
  final_pressure_analog = analogRead(pressure_transducer_2);
  experimental_flow_rate_analog = analogRead(flow_rate_sensor);


  voltage = (float) (intial_pressure_analog / 1023.0) * 5.0;
  pressure_psi = (voltage / 5.0 ) * 15.0;
  pressure_mmHg = pressure_psi * 57.7149;
  initial_pressure_pa = pressure_psi * 6894.7572932;


  voltage = (float) (final_pressure_analog / 1023.0) * 5.0;
  pressure_psi = (voltage / 5.0 ) * 15.0;
  pressure_mmHg = pressure_psi * 57.7149;
  final_pressure_pa = pressure_psi * 6894.7572932;
  pressure_difference = abs(final_pressure_pa - initial_pressure_pa);

  voltage = (float) (experimental_flow_rate_analog / 1023.0) * 5.0;
  experimental_flow_rate = (196.46 * voltage) + 7.9756;

  theoretical_flow_rate = (float) ((((pressure_difference) * PI * pow(tubing_radius, 4)) / (8.0 * water_viscosity * tubing_length)) / pow(1000.0, 3)) * 60.0;

  /*
  Serial.print("Initial Pressure = ");
  Serial.print(initial_pressure_pa);
  Serial.print(" : ");
  Serial.print("Final Pressure = ");
  Serial.print(final_pressure_pa);
  //Serial.println();
  Serial.print(" : ");
  /*
  ...
  Serial.println(" pa");
  Serial.print("Theoretical Flow Rate = ");
  Serial.print(theoretical_flow_rate);
  Serial.println(" mL/min");
  */
  //Serial.print("Experimental Flow Rate = ");
  Serial.println(experimental_flow_rate);
  
  /*
  Serial.print("Percent Error ");
  Serial.print(((pressure_difference) / theoretical_flow_rate) * 100);
  Serial.println(" %");
  */
